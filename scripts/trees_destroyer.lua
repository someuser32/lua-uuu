...
-- 	-- self.tree_destroyer_enable = UILib:CreateCheckbox(self.tree_destroyer, "Enable", false)
-- 	-- self.tree_destroyer_enable:SetCallback(function(enable)
-- 	-- 	if enable:Get() then
...
-- 	-- self.tree_destroyer_panel = UILib:CreateKeybind(self.tree_destroyer, "Panel Key")
-- 	-- self.tree_destroyer_panel:Icon("")
...
-- 	-- self.tree_destroyer_notifications = UILib:CreateCheckbox(self.tree_destroyer, "Notifications", true, "")

-- 	-- self.tree_destroyer_notifications_gear = self.tree_destroyer_notifications:Gear("Notifications")
-- 	-- self.tree_destroyer_notifications_text = UILib:CreateCheckbox(self.tree_destroyer_notifications_gear, "Text", true, "")
-- 	-- self.tree_destroyer_notifications_sound = UILib:CreateCombo(self.tree_destroyer_notifications_gear, "Sound", {"Disabled"}, "")
-- 	-- self.tree_destroyer_notifications_sound_volume = UILib:CreateSlider(self.tree_destroyer_notifications_gear, "Volume", 1, 100, 50, "%d%%", "")
...
-- 	-- self.panel_border_size = 1
-- 	-- self.panel = Widget:from_config(300, 400, "tree_destroyer", "magma_tree_destroyer", Vec2(self.panel_border_size, self.panel_border_size))
end
...
-- -- function TreeDestroyerExtended:OnDraw()
-- -- 	if not self.tree_destroyer_enable:Get() then return end
-- -- 	if self.tree_destroyer_panel:IsToggled() then
-- -- 		self.panel:Render(CInput:IsKeyDown(Enum.ButtonCode.KEY_MOUSE1))

-- -- 		CRender:Rect(Vec2(self.panel.pos.x-self.panel_border_size, self.panel.pos.y-self.panel_border_size), Vec2(self.panel.pos.x+self.panel.width+self.panel_border_size, self.panel.pos.y+self.panel.height+self.panel_border_size), Menu.Style("outline"), 2)
-- -- 		CRender:Shadow(Vec2(self.panel.pos.x-5, self.panel.pos.y-5), Vec2(self.panel.pos.x+self.panel.width+5, self.panel.pos.y+self.panel.height+5), Menu.Style("shadow"), 1.0)
-- -- 		CRender:FilledRect(self.panel.pos, Vec2(self.panel.pos.x+self.panel.width, self.panel.pos.y+self.panel.height), Menu.Style("main_background"), 2)

-- -- 		CRender:Text(self.icons, 12, "", Vec2(self.panel.pos.x+5, self.panel.pos.y+4), Menu.Style("primary"))
-- -- 		CRender:Text(self.font, 12, "Tree Destroyer", Vec2(self.panel.pos.x+5+12+3, self.panel.pos.y+4), Menu.Style("section_group_text"))

-- -- 		CRender:FilledRect(Vec2(self.panel.pos.x, self.panel.pos.y+4+12+4), Vec2(self.panel.pos.x+self.panel.width+2, self.panel.pos.y+4+12+6), Menu.Style("outline"))
...
-- -- 		local owner_image = CRender:GetOrLoadImage(GetHeroIconPath(owner:GetUnitName()))
-- -- 		local tree_image = CRender:GetOrLoadImage(CAbility:GetAbilityNameIconPath(tree_type))
-- -- 		local caster_image = CRender:GetOrLoadImage(GetHeroIconPath(destroy_ability:GetCaster():GetUnitName()))
-- -- 		local destroy_image = CRender:GetOrLoadImage(CAbility:GetAbilityNameIconPath(destroy_ability:GetName()))
-- -- 		CRender:DrawCenteredNotification("{#FF0000}[{&"..owner_image.."}{#FF0000}]{&"..tree_image.."}{#FFFFFF} destroy by {#00FF00}[{&"..caster_image.."}{#00FF00}]{&"..destroy_image.."}", 2)
-- -- 	end
...
-- -- 	if NPC.HasModifier(hero, "modifier_bloodseeker_rupture") then
-- -- 		return false
-- -- 	end
-- -- 	return true
-- -- end
