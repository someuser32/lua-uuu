...
function localize_hero(name)
	...
	return t[name] or string.sub(name, 15)
end

function getcircle(x, y, r, points)
	local t = {}
	for i = 1, points or 360 do
		local angle = i * math.pi / 180 * 360/points
		local ptx, pty = x + r * math.cos(angle), y + r * math.sin(angle)
		table.insert(t, {ptx, pty})
	end
	return t
end
...
function DeepPrintTable(t)
	...
	for k, v in pairs(t) do
		Log.Write("\t"..tostring(k).."\t\t"..tostring(v).."\t"..type(v))
	end
	Log.Write("}")
end
