...
function print(...)
    local text = table.concat(tablemap({...}, function(k, v) return v ~= nil and tostring(v) or "nil" end), "\t")
    Log.Write(text ~= nil and text or "nil")
end
...
function _DeepPrintTable(debugInstance, prefix, isOuterScope, chaseMetaTables )
    ...
    if isOuterScope then  -- special case for outer call - so we dont end up iterating strings, basically
        if type(debugInstance) == "table" then
            ...
            prefix = prefix .. "   "
			terminatescope = true
        elseif type(debugInstance) == "function" then
            print( prefix .. " = " .. print_func(debugInstance))
        else
    ...
    if type(debugOver) == "table" then
        for idx, data_value in pairs(debugOver) do
            ...
            elseif type(data_value) == "function" then
                print( string.format( "%s%-32s\t= %s (%s)", prefix, idx, print_func(data_value), type(data_value) ) )
            else
                print( string.format( "%s%-32s\t= %s (%s)", prefix, idx, tostring(data_value), type(data_value) ) )
            end
        end
    elseif type(debugOver) == "function" then
        print( string.format( "%s%-32s\t= %s (%s)", prefix, idx, print_func(debugOver), type(debugOver) ) )
    else
        print( string.format( "%s%-32s\t= %s (%s)", prefix, idx, tostring(debugOver), type(debugOver) ) )
    end
