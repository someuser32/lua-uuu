...
function timer.Barebones(delay, func, context)
	local callback = function(context, call)
		return function()
			local delay_new = func(context)
			if type(delay_new) == "number" then
				timer.Simple(delay_new, call(context, call))
			end
		end
	end
	return timer.Simple(delay, callback(context, callback))
end